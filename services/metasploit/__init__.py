...
class MetasploitScriptHandlingService(IScriptHandlingService):
    ...
    def execute_script(self, args) -> bool:
        ...
        try:
            ...
            platform = "windows"
            self._logger.info(
                f"Getting potential exploits from Metasploit (platform: {platform})..."
            )
            search_results = self._get_exploits(
                platform=platform, from_date=args.ms_from_date, to_date=args.ms_to_date
            )
            ...
            for search_result in search_results:
                _, module_path = MetasploitHelper.get_module_type(
                    search_result.fullname
                )
                module = self._client.modules.use("exploit", module_path)

                if not self._is_supported_module(module.options):
                    self._logger.debug(
                        f"Module {search_result.fullname} cannot automatically deploy as a malicious webserver. Skipping..."
                    )
                else:
            ...
            for exploit_module in final_results:
                ...
                # Run the audit
                result = vbox_service.run_workflow(config=config)
                self._logger.info(
    ...
    def _deploy_malicious_webserver(
        self,
        module_fullname: str,
        host: str = DEFAULT_MS_HOST,
        lport: int = DEFAULT_MS_LPORT,
        srvhost: str = DEFAULT_MS_SRVHOST,
        srvport: int = DEFAULT_MS_SRVPORT,
        uri_path: str = DEFAULT_MS_URIPATH,
        payload: str = DEFAULT_MS_PAYLOAD,
    ) -> MaliciousWebServerMetadata:
        ...
        if job_id is None:
            raise ValueError(
                f"Failed to execute the exploit {module_fullname} with payload {payload}."
            )
    ...
    def _is_supported_module(self, module_options: list[str]):
        supported_opts = [
            ...
            "SRVPORT",
            "URIPATH",
        ]  # Requires to deploy webserver automatically
        contains_all_supported_opts = all(
            option in module_options for option in supported_opts
        )

        unsupported_opts = ["RHOST", "RHOSTS", "RPORT"]
        contains_any_unsupported_opts = any(
            option in module_options for option in unsupported_opts
        )
    ...
    def _get_exploits(
        ...
        raw_results = self._client.call(
            "module.search", [f"type:exploit platform:{platform}"]
        )
        results: list[MetasploitSearchResult] = [
            MetasploitSearchResult(**e) for e in raw_results if type(e) is dict
        ]
        ...
        if from_date is not None:
            sorted_results = [
                ...
                if datetime.datetime.strptime(e.disclosuredate, "%Y-%m-%d") >= from_date
            ]
        if to_date is not None:
            sorted_results = [
                e
                for e in sorted_results
                if datetime.datetime.strptime(e.disclosuredate, "%Y-%m-%d") <= to_date
            ]
